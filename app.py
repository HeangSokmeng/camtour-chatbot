...
# Load the travel data from domain
try:
...
def clean_text(text):
    ...
    # Remove punctuation except important ones for travel (like $, %)
    text = re.sub(r'[^\w\s$%]', ' ', text)
    
    # Remove extra whitespace
    text = ' '.join(text.split())
    
    # Process with spaCy for lemmatization and stop word removal
    doc = nlp(text)
...
def process():
    ...
    # Check if travel data is available
    ...
    # Clean user message for similarity matching
    ...
    # Get the most similar question and confidence score
    max_sim_idx = similarities.argmax()
    confidence = float(similarities[max_sim_idx])
    
    # Dynamic response based on confidence level
    if confidence < 0.2:
        ...
        # Low confidence - provide answer but suggest alternatives
        ...
        # Good confidence - provide the answer
        return jsonify({
...
def get_top_suggestions():
    """Get most popular/important suggestions"""
    return [
...
def get_related_suggestions(main_idx):
    """Get suggestions related to the matched question"""
    try:
        ...
        if len(location_questions) > 1:
            # Sample related questions (excluding the current one)
        ...
        # If no location-based suggestions, get general suggestions
...
def get_suggestions():
    """Endpoint to get suggestions without processing a message"""
    return jsonify({
        'suggestions': get_top_suggestions()
    })
